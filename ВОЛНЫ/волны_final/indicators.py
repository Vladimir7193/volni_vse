    ...
    while i < n:
        ...
        if last_pivot_type == 'high':
            ...
            if last_pivot_price > 0 and (last_pivot_price - window_low) / last_pivot_price >= deviation_pct:
                # Проверяем, нет ли более высокого хая перед этим лоу
...
def find_zigzag_legs(pivots: list[dict]) -> list[dict]:
    """
    Преобразует пивоты ZigZag в 'ноги' (legs) — направленные движения.
    Каждая нога: {'start': pivot, 'end': pivot, 'direction': 'up'|'down', 'size': float, 'size_pct': float}
    """
    legs = []
...
def get_zigzag_for_timeframe(df: pd.DataFrame, timeframe: str) -> tuple[list[dict], list[dict]]:
    """
    Получить ZigZag пивоты и ноги для конкретного таймфрейма
    """
